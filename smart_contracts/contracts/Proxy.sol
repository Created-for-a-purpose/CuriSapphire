...
interface IData {
  ...
  function getPatientData(address patient) external view returns (patientData memory);
  ...
  function proxy_addPharmacyData(string memory _name, string memory _license, string memory _address, string memory _contact, string memory _email) external ;
  function proxy_addHospitalData(string memory _name, string memory _license, string memory _address, string memory _contact, string memory _email) external ;
}
...
contract GaslessTx {
  ...
  function checkValidity(bytes memory signature, address owner) internal pure returns(bool){
  ...
  function makeDataTx(bytes memory signature, txRequest memory _txRequest) verifiedOnly(signature, _txRequest.owner)
  external view returns (bytes memory){
      // hash request
      bytes32 requestHash = keccak256(abi.encodePacked(
       _txRequest.owner, _txRequest.name, _txRequest.age, _txRequest.bloodGroup, _txRequest._address, _txRequest.contact, _txRequest.email
      ));
      // encrypt request
      bytes32 ciphertextnonce = keccak256(abi.encodePacked(
        encryptionSecret, requestHash
      ));
      // inner call
      bytes memory innercall = abi.encodeWithSelector(dataProxy.proxy_addPatientData.selector,
        _txRequest.owner,
        _txRequest.name, _txRequest.age, _txRequest.bloodGroup, _txRequest._address, _txRequest.contact, _txRequest.email);
      // encrypt with sapphire
      bytes memory ciphertext = Sapphire.encrypt(
        encryptionSecret, ciphertextnonce, 
